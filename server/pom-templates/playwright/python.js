// server/pom-templates/playwright/python.js
// This file provides the AI Generation Guide content for Playwright Python (using pytest).
...
module.exports = (projectName, dynamicBaseUrl) => { 
    ...
    const aiGenerationGuideContent = `## AI Code Generation Guide (Playwright Python - Pytest - Flat Top-Level POM)
...
1.  **Project Structure Adherence (Playwright Python Specific):**
    * **IMPORTANT: The following base project files are provided by the server and DO NOT need to be generated by you:** \\\`pyproject.toml\\\`, \\\`conftest.py\\\`, \\\`utils/browser_manager.py\\\`, \\\`.env\\\`, \\\`.gitignore\\\`, \\\`README.md\\\`, \\\`requirements.txt\\\`. **DO NOT include these in your JSON output.**
...
    * This module should define Playwright \\\`Locator\\\` instances or functions returning locators (e.g., \\\`USERNAME_FIELD = "input[name='username']"\\\` or \\\`def get_username_field(page): return page.locator("input[name='username']")\\\`). Prioritize string selectors for direct use with \\\`page.locator()\\\`.
...
    * **Methods:** Implement high-level methods corresponding to user actions/flows using the \\\`self.page\\\` object and imported locators (e.g., \\\`self.page.fill(LoginLocators.USERNAME_FIELD, username)\\\`).
...
    * It MUST import \\\`pytest\\\` and \\\`expect\\\` from Playwright.
...
    * Test functions/methods **MUST** accept a \\\`page\\\` fixture as an argument (e.g., \\\`def test_something(page):\\\`).
    * Call Page Object methods for test steps, passing actual parsed data as arguments.
    * Implement assertions based on "Expected Result" using Playwright's \\\`expect()\\\` (e.g., \\\`expect(page.locator("#someElement")).to_be_visible()\\\`).
