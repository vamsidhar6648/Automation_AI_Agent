// server/pom-templates/selenium/java.js
// This file provides the AI Generation Guide content for Selenium Java (using JUnit 5 and Maven).
...
module.exports = (projectName, dynamicBaseUrl) => { 
    ...
    const aiGenerationGuideContent = `## AI Code Generation Guide (Selenium Java - JUnit 5 - Maven - Flat Top-Level POM)
...
1.  **Project Structure Adherence (Selenium Java Specific):**
    * **IMPORTANT: The following base project files are provided by the server and DO NOT need to be generated by you:** \\\`pom.xml\\\`, \\\`src/main/java/base/BaseTest.java\\\`, \\\`src/main/java/utils/WebDriverSetup.java\\\`, \\\`.env\\\`, \\\`.gitignore\\\`, \\\`README.md\\\`. **DO NOT include these in your JSON output.**
...
        * **NO separate WebActions file generation:** Common WebDriver actions are in \\\`BaseTest.java\\\` or directly in Page Objects.
...
    * This class should define static \\\`By\\\` locators (e.g., \\\`public static By USERNAME_FIELD = By.id("username");\\\`).
...
    * This class **MUST** have a constructor that accepts \\\`WebDriver driver\\\` and stores it.
    * It **MUST import its corresponding Locator class** (e.g., \\\`import pageobjects.LoginLocators;\\\`).
    * **Methods:** Implement high-level methods corresponding to user actions/flows using the \\\`WebDriver driver\\\` and imported locators (e.g., \\\`driver.findElement(LoginLocators.USERNAME_FIELD).sendKeys(username);\\\`).
    * **Assertions:** Page Object methods can return data or perform basic assertions if appropriate.
...
    * Implement assertions based on "Expected Result" using JUnit \\\`Assertions\\\` (e.g., \\\`Assertions.assertEquals(expected, actual);\\\`).
